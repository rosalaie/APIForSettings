...
export default function Loja() {
  ...
  async function carregarProdutos() {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        setProdutoSelecionado(data[0]); // Seleciona o primeiro produto por padrão para o painel lateral
      }
    } catch (err) {
  ...
  return (
    <div style={{ backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      {/* TOPO DO SITE DA LOJA (ESTILO IMAGEM SOBMDIDA) */}
      <header style={{
        background: '#ffffff',
      ...
      <div style={{ maxWidth: '1280px', margin: '30px auto', padding: '0 20px' }}>
        ...
        {/* LAYOUT PRINCIPAL: GRADE + DETALHES LATERAL */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: '25px', alignItems: 'start' }}>
                ...
                {produtosFiltrados.map(prod => (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      {prod.imagemUrl ? (
                        <img
                          src={`http://localhost:3000${prod.imagemUrl}`}
                          alt={prod.nome}
          ...
          {/* DETALHES DO PRODUTO SELECIONADO (PAINEL DIREITO) */}
          {produtoSelecionado ? (
            <div style={{
              background: '#fff',
              borderRadius: '12px',
              ...
              <div style={{
                ...
                {produtoSelecionado.imagemUrl ? (
                  <img
                    src={`http://localhost:3000${produtoSelecionado.imagemUrl}`}
